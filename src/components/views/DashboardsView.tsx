      ...
      <div className="grid grid-cols-3 gap-5 items-start">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm space-y-3">
            <div className="flex justify-between items-center pb-3 border-b border-slate-50">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-display font-semibold text-sm text-slate-800">Time Blocks Today</h3>
              </div>
              <span className="text-[10px] font-bold text-slate-400 bg-slate-50 border border-slate-100 px-2.5 py-0.5 rounded-lg">{initialTimeLogs.length} Blocks</span>
            </div>
                  ...
                  <div key={log.id} className="group p-3.5 rounded-xl border border-slate-100 bg-white hover:bg-slate-50/40 hover:shadow-sm transition-all">
                    <div className="flex justify-between items-start gap-3">
                      <div className="flex items-start gap-2.5">
                        <div className="w-7 h-7 rounded-lg bg-slate-50 border border-slate-100 text-slate-400 flex items-center justify-center">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm space-y-3">
            <div className="flex justify-between items-center pb-3 border-b border-slate-50">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-display font-semibold text-sm text-slate-800">Activity Logs</h3>
              </div>
            </div>
              ...
              {initialActivityHistory.map(item => {
                ...
                return (
                  <div key={item.id} className="relative group">
                    <span className={`absolute -left-[29px] top-1.5 w-4 h-4 rounded-full flex items-center justify-center ${isDayIn ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-500'}`}>
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="text-xs font-semibold text-slate-800">
                            {item.user} 
                            <span className={`ml-1.5 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase ${isDayIn ? 'bg-emerald-500/10 text-emerald-600' : 'bg-rose-500/10 text-rose-600'}`}>
                              {item.action}
                            </span>
                          </p>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm space-y-3">
            <div className="flex justify-between items-center pb-3 border-b border-slate-50">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-display font-semibold text-sm text-slate-800">Effort Breakdown</h3>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm space-y-3">
            <div className="flex justify-between items-center pb-3 border-b border-slate-50">
              <div className="flex items-center gap-2">
                <div className="p-1.5 rounded-lg bg-rose-50 text-[#FF6347]">
                  <Briefcase className="w-4 h-4" />
                </div>
                <h3 className="font-display font-semibold text-sm text-slate-800">Watchlist</h3>
              </div>
              <button onClick={() => setShowAddProjectModal(true)} className="p-1.5 bg-[#1490FE] hover:bg-[#0070DF] text-white rounded-lg shadow-sm transition-all">
              ...
              {watchlist.map(proj => (
                <div key={proj.id} className="p-3 rounded-xl border border-slate-100 hover:bg-slate-50/40 transition-all space-y-2.5">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => setWatchlist(watchlist.filter(p => p.id !== proj.id))} className="p-1 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors">
                      <MinusCircle className="w-3.5 h-3.5" />
                    </button>
                  </div>
                  <div className="flex items-center justify-between border-t border-slate-50 pt-2">
                    {[
                      { Icon: Copy, fn: () => handleCopy(proj.name), title: 'Copy' },
                      { Icon: List, fn: () => {}, title: 'Tasks' },
                      { Icon: ClipboardList, fn: () => {}, title: 'Logs' },
                      { Icon: ChevronRight, fn: () => {}, title: 'Details', color: 'text-[#1490FE]' }
                    ].map(({ Icon, fn, title, color }, i) => (
                      <button key={i} onClick={fn} title={title} className={`p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-50 rounded-md transition-colors ${color || ''}`}>
